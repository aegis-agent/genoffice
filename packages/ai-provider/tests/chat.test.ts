...
describe('chatForProvider', () => {
  ...
  it('custom: fail-closes without an explicit customFetch and never calls global fetch', async () => {
    ...
    expect(result.ok).toBe(false)
    expect(result.error).toMatch(/customFetch/i)
    expect(fetchMock).not.toHaveBeenCalled()
  })

  it('custom: routes through the supplied customFetch, not global fetch', async () => {
  ...
  it('custom: rejects without a base URL even when customFetch is supplied', async () => {
